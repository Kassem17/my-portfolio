...
export default function Home() {
  ...
  const handleDownload = () => {
    const cvHref = homeData.buttons?.[0]?.href || homeData.cvUrl;
    if (!cvHref || cvHref === "#") {
      Swal.fire({
    ...
    Swal.fire({
      title: "Download CV?",
      text: "Get my resume to know more about me.",
      icon: "question",
  ...
  useEffect(() => {
    const titlesList = homeData.typingTexts || homeData.titles || [];
    if (!titlesList.length) return;
    const currentTitle = titlesList[currentIndex];
    ...
    const timer = setTimeout(() => {
      if (!isDeleting) {
        ...
        if (currentText.length > 0) {
          ...
          setIsDeleting(false);
          setCurrentIndex((prev) => (prev + 1) % titlesList.length);
        }
      }
    }, typeSpeed);
        ...
        <div className="grid grid-cols-12 gap-5 lg:gap-6 min-h-[calc(100vh-8rem)]">
          ...
          <div className="col-span-12 lg:col-span-7 flex flex-col gap-5">
            ...
            <SectionReveal
              variant="fade-up"
              delay={0.05}
              className="bento-card p-6 lg:p-8 rainbow-card"
          ...
          <div className="col-span-12 lg:col-span-5 flex flex-col gap-5">
            ...
            <SectionReveal
              variant="slide-right"
              delay={0.2}
              className="bento-card rainbow-card relative overflow-hidden"
              style={{
